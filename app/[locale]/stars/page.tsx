...
// ✅ المشكلة كانت هنا — useState و useEffect كانوا جوا .map()
// الحل: مكون منفصل بيستقبل البيانات ويعمل الـ state بتاعته لوحده
type SectorWinnerRowProps = {
  sector: Sector;
  rank: 1 | 2;
...
function SectorWinnerRow({
  ...
  onSave,
  onDelete
}: SectorWinnerRowProps) {
  const [name, setName] = useState(initialRow?.name ?? "");
  const [title, setTitle] = useState(initialRow?.title ?? "");
  const [photo, setPhoto] = useState(initialRow?.photo_url ?? "");
  const [profile, setProfile] = useState(initialRow?.profile_url ?? "");

  // ✅ useEffect هنا صح — على مستوى المكون مش جوا loop
  useEffect(() => {
    setName(initialRow?.name ?? "");
    setTitle(initialRow?.title ?? "");
    setPhoto(initialRow?.photo_url ?? "");
    setProfile(initialRow?.profile_url ?? "");
  }, [initialRow, month]);
...
export default function AdminStarsPage() {
  ...
  // Map لتسهيل الوصول للبيانات بالـ key
  const byKey = useMemo(() => {
  ...
  const handleUpsert = async (payload: Omit<Row, "id"> & { id?: string }) => {
    setErrorMsg("");
    const session = await requireAllowedSession();
    if (!session) {
    ...
    const { error } = await supabaseBrowser.from("sector_stars").upsert({
      id: payload.id,
      month: payload.month,
          ...
          {SECTORS.map((s) => (
            ...
            >
              ...
              <div className="mt-4 grid gap-4">
                {/* ✅ مكون منفصل — مفيش hooks جوا map هنا */}
                <SectorWinnerRow
                  sector={s}
                  rank={1}
                  initialRow={byKey.get(`${s.slug}:1`)}
                  month={month}
                  isAr={isAr}
                  onSave={handleUpsert}
