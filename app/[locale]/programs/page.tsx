...
export default async function ProgramsPage({
  ...
  const softPill =
    "rounded-full border border-black/10 bg-white/90 px-3 py-1 text-xs text-zinc-600 dark:border-white/10 dark:bg-zinc-950/45 dark:text-zinc-300";

  const primaryBtn =
    "inline-flex items-center justify-center rounded-2xl bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-sky-700 active:scale-[0.99] dark:bg-amber-400 dark:text-zinc-950 dark:hover:bg-amber-300";
                ...
                {featuredPrograms.map((program) => {
                  ...
                  return (
                    <MotionDiv key={program.id} variants={fadeUp}>
                      ...
                      >
                        <div className="relative aspect-[16/9] w-full bg-zinc-50 dark:bg-white/5">
                          {program.cover_url ? (
                            <img
                        ...
                        <div className="grid gap-3 p-4">
                          <p className="line-clamp-3 text-sm leading-7 text-zinc-600 dark:text-zinc-300">
                            {excerpt ||
                              (isAr
                                ? "وصف البرنامج قيد الإعداد."
                                : "Program description is coming soon.")}
                          </p>

                          <div className="mt-auto flex items-center justify-between gap-3">
                            <span className="text-sm font-semibold text-sky-700 dark:text-amber-200">
            ...
            <section className="grid gap-4">
              ...
              >
                {listingPrograms.map((program) => {
                  ...
                  return (
                    <MotionDiv key={program.id} variants={fadeUp}>
                      <article className={cx("group overflow-hidden", glass, cardHover)}>
                        <div className="relative aspect-[16/9] w-full bg-zinc-50 dark:bg-white/5">
                          {program.cover_url ? (
                            <img
                              src={program.cover_url}
