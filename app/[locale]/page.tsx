...
export default async function HomePage({
  ...
  // ✅ عرض الكارت في الـ slider — 85% من عرض الشاشة على الموبايل، 300px على الـ desktop
  const sliderCard =
    "group block w-[85vw] max-w-[340px] min-w-[240px] shrink-0 snap-start overflow-hidden rounded-[28px] border border-black/10 bg-white/78 backdrop-blur-xl dark:border-white/10 dark:bg-zinc-900/60 transition duration-200 hover:-translate-y-1 hover:border-black/15 hover:shadow-lg";
  ...
  const monthText = latestAwards
    ? formatMonth(safeLocale, latestAwards.award_month)
    : "";
        ...
        {featuredPrograms.length === 0 ? (
          ...
          <HomeHorizontalSlider>
            {featuredPrograms.map((program) => {
              const title =
                pickText(isAr, program.title_ar, program.title_en) || "—";
        ...
        {featuredEvents.length === 0 ? (
          ...
          <HomeHorizontalSlider>
            {featuredEvents.map((event) => {
              const title =
                pickText(isAr, event.title_ar, event.title_en) || "—";
              const desc = stripHtml(
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_320px]">
              ...
              <div className={cx("p-4", softCard)}>
                ...
                <Link
                  ...
                  className={cx("mt-4 inline-flex", primaryBtn)}
                >
